...
- Shift 달리기, Space 점프, 모바일 더블탭 점프, 한양대 로고 부유·회전 연출
...
## 파일 구조
    ...
    ├── style.css        # 라이트/다크/3D 연구 공간, 독립 스크롤, 반응형, 인쇄 스타일
...
- Shift를 누르면 달리고, Space로 점프합니다. 모바일에서는 320ms 이내 화면 더블탭으로 점프합니다.
...
- 3D 모던 연구 공간에서 WASD·마우스 이동, Shift 달리기, Space/모바일 더블탭 점프, 시작 위치 초기화, 7개 전시대 열기 확인
