...
- 반응형 앵커 이동, 독립 스크롤, 언어·테마 전환, 모던 하우스형 3D 연구 공간 탐색·전시대, 이미지 확대, 인쇄
...
| UR-05 | 모던 하우스형 3D 연구 공간은 로봇 모형·전시대·HUD를 포함하고 모션 감소 및 터치 조작을 지원한다. | 완료 |
...
| FR-02 | 전체 한·영, 라이트/다크, 기본 CV/3D 연구 공간 전환값을 `localStorage`에 저장한다. | 완료 |
| FR-03 | WebGL 실패 시 3D 버튼을 숨기고 기본 CV를 유지한다. | 완료 |
| FR-04 | 모던 하우스형 3D 연구 공간에서 WASD·마우스 또는 터치로 이동하고 7개 전시대에서 CV 섹션을 연다. | 완료 |
...
| 2026-08-10 | v1.2 | 모던 하우스형 3D 연구 공간의 전시대 분리 배치와 공간 디자인 반영 |
