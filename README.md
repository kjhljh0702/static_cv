...
MyHub는 채용 담당자와 협업 파트너를 위한 단일 페이지 웹 CV입니다. 프레임워크나 빌드 도구 없이 HTML, CSS, Vanilla JavaScript로 동작하며, 모든 이력서 콘텐츠는 하나의 data.json에서 한글과 영어로 관리합니다. 선택형 3D 배경에는 저장소에 포함된 Three.js를 사용합니다.

## 주요 기능

- 한글/영어 전체 전환 및 마지막 선택 언어 저장
- 라이트 모드 기본, 다크 모드 전환 및 선택값 저장
- 기본 CV/3D 메타버스 보기 전환 및 선택값 저장
...
## 파일 구조
    ...
    ├── style.css        # 라이트/다크/3D, 독립 스크롤, 반응형, 인쇄 스타일
    ├── script.js        # JSON 로드, 렌더러, 언어/테마/메뉴/갤러리 동작
    ├── metaverse.js     # Three.js 3D 장면과 보기/테마 동기화
...
- 상단 큐브 아이콘으로 기본 CV와 3D 메타버스 보기를 전환합니다.
- 선택값은 myhub-view-mode 키로 localStorage에 저장됩니다.
- 3D 오브젝트, 파티클 수, 카메라 움직임은 metaverse.js에서 수정합니다.
- 3D 모드의 반투명 패널과 HUD 스타일은 style.css의 data-view="metaverse" 선택자에서 수정합니다.
- 모션 감소 설정(prefers-reduced-motion)이 켜진 환경에서는 애니메이션을 최소화합니다.
